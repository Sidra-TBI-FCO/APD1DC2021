...
common_colnames <- colnames(dfs[[2]])
for (i in 3:length(dfs))
{
  common_colnames <- intersect(common_colnames,colnames(dfs[[i]]))
}
dataset_ids <- c(1:7)
...
save(list=c("req_columns","gbmFit","svmFit","rdaFit","gpFit","rfFit","xgbFit"),file="Model_building/Required_Files/CV_ML_models_submission_q3_v3.Rdata")
